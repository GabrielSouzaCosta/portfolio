"""Morfeu v03: photo-informed anatomy, continuous surfaces and actual eyelids.
...
body_masses=[trunk,mass('Sacral tail transition',(0,.579,.715),(.108,.14,.113))]
for s,label in [(-1,'L'),(1,'R')]:
 shoulder=(s*.255,-.02,1.56);elbow=(s*.285,-.18,.99);wrist=(s*.265,-.34,.27)
 arm=tube('Continuous foreleg '+label,[(s*.145,.08,1.69),(s*.275,-.1,1.29),(s*.282,-.18,1.02),(s*.271,-.275,.62),(s*.265,-.35,.265),(s*.265,-.42,.13)],[.14,.126,.102,.09,.081,.115],cream,group='sculpt',sides=20)
 body_masses += [mass('Shoulder blend '+label,(s*.20,.015,1.48),(.16,.22,.23)),arm,mass('Forepaw '+label,(s*.265,-.423,.112),(.137,.199,.098))]
 for k in range(4):body_masses.append(mass('Foretoe '+label+str(k),(s*.265+(k-1.5)*.058,-.568+abs(k-1.5)*.013,.086),(.044,.067,.056)))
 body_masses += [mass('Folded thigh '+label,(s*.343,.27,.56),(.222,.302,.315)),limbmass('Folded calf '+label,(s*.385,.015,.50),(s*.413,.30,.215),.093,.105),limbmass('Hind metatarsus '+label,(s*.413,.30,.215),(s*.451,.016,.108),.088,.095),mass('Hind paw '+label,(s*.451,-.006,.101),(.12,.204,.087))]
...
head_masses=[mass('Cranium',(0,.02,2.30),(.535,.395,.405)),mass('Occiput',(0,.13,2.20),(.40,.345,.36)),mass('Jaw',(0,-.09,2.105),(.385,.31,.24)),mass('Nasal bridge',(0,-.285,2.19),(.13,.225,.22)),mass('Chin',(0,-.348,1.989),(.164,.177,.092))]
for s in [-1,1]:head_masses += [mass('Cheek',(s*.255,-.08,2.16),(.287,.302,.212)),mass('Whisker pad',(s*.108,-.369,2.061),(.182,.179,.12))]
...
def bodycolor(p):
 ...
 b=((x+.035)/.67)**2+((y-.31)/.46)**2+((z-.68)/.46)**2
 patch=max(1-smooth(.78,1.05,a),1-smooth(.83,1.12,b))
 gap=abs(z-(1.075+.14*x+.045*math.sin(x*8)))
 patch*=smooth(.045,.092,gap)*smooth(.005,.10,y)*smooth(.29,.4,z)
...
EYE_W=.198;EYE_H=.167;EYE_Z=2.303
lids=[]
...
for j in range(IH):
 ...
 for i in range(IW):
  ...
  c=blend((.77,.72,.28,1),(.49,.59,.22,1),.27+.09*math.sin(t*38))
  c=blend(c,(.36,.31,.12,1),smooth(.83,.98,r)*.54)
  c=blend((.055,.052,.033,1),c,smooth(.97,1.035,q))
...
for s,label in [(-1,'L'),(1,'R')]:
 ...
 def point(dx,dz,extra=0):
  x=cx+dx;z=EYE_Z+dz+s*.095*dx
  return (x,y0+ax*dx+az*dz+qx*dx*dx+qz*dz*dz-extra,z)
 vs=[];fs=[];uvs=[];N=80;R=22
 for j in range(R+1):
  r=j/R
  for i in range(N+1):
   t=math.tau*i/N;dx=EYE_W*r*math.cos(t);dz=EYE_H*r*math.copysign(abs(math.sin(t))**1.6,math.sin(t))
   vs.append(point(dx,dz));uvs.append((.5+dx/(2*EYE_W),.5+dz/(2*EYE_H)))
 for j in range(R):
 ...
 ring=[]
 for i in range(80):
  t=math.tau*i/80;dx=EYE_W*math.cos(t);dz=EYE_H*math.copysign(abs(math.sin(t))**1.6,math.sin(t));x,y,z=point(dx,dz)
 ...
 for upper in [True,False]:
  ...
  for j,v in enumerate(rows):
   for i in range(NX+1):
    u=i/NX*2-1;dx=EYE_W*u;edge=EYE_H*max(0,1-u*u)**.8*(1 if upper else -1)
    openz=edge+(1-v)*(.016 if upper else -.012)
    # The closed skin overlaps the entire socket boundary. The last tightly
    # spaced rows form one fine curved crease, rather than an open-eye ring.
    closez=(edge+(.018 if upper else -.015))*(1-v)+(-.018*math.sqrt(max(0,1-u*u)))*v
    vs.append(point(dx,openz,.003));closed.append(point(dx*1.04,closez,.009))
  for j in range(NY):
...
for s,label in [(-1,'L'),(1,'R')]:
 ...
 for k in range(3):
  ...
  tube('Whisker '+label+str(k),[start,(s*.43,-.42,2.09-k*.06),(s*(.70-k*.025),-.34,2.13-k*.087)], [.0048,.0033,.0008],cream,bone='head',sides=6)
...
export('morfeu-rigged-v03.glb',[rig]+catparts)
rig.scale=(.44,)*3;rig.location=(0,.22,.59)
export('morfeu-scout-v03.glb',[rig]+catparts+[shiproot]+ship)
...
for name,loc in [('portrait',(3.1,-7,3.0)),('front',(0,-8,2.5)),('side',(7,-.7,2.8)),('rear',(4,7,3.1))]:
 cam.location=loc;aim(cam,(0,0,1.49));scene.render.filepath=str(P/'previews'/f'morfeu-{name}.png');bpy.ops.render.render(write_still=True)
# Inspect an actual closed-lid frame without changing or squashing the eyeballs.
scene.frame_set(52);cam.location=(0,-8,2.6);aim(cam,(0,0,1.5));scene.render.filepath=str(P/'previews/morfeu-blink.png');bpy.ops.render.render(write_still=True);scene.frame_set(1)
rig.scale=(.44,)*3;rig.location=(0,.22,.59)
...
bpy.ops.wm.save_as_mainfile(filepath=str(P/'morfeu-photo-rigged.blend'))
