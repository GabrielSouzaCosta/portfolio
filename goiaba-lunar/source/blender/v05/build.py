"""Morfeu v05: orange eyes, warmer expression, and the original Semente scout.
...
for j in range(IH):
 ...
 for i in range(IW):
  ...
  q=((u-.483)/.315)**2+((v-.543)/.358)**2
  c=blend(rgba('FFAF43'),rgba('CF651A'),.40+.055*math.sin(t*38))
  c=blend(c,rgba('9A3C11'),smooth(.83,.98,r)*.54)
  c=blend(rgba('191C18'),c,smooth(.97,1.035,q))
  c=blend(c,rgba('71321A'),smooth(.962,.995,r))
  if (.315<u<.465 and .658<v<.818) or (.662<u<.713 and .281<v<.338):c=(1,.988,.949,1)
  pixels.extend(c)
tex=bpy.data.images.new('Morfeu orange eyes and sprite catches',IW,IH);tex.pixels=pixels;tex.filepath_raw=str(P/'morfeu-eyes.png');tex.file_format='PNG';tex.save();tex.pack()
eye_material=mat('Morfeu | warm orange eyes','gold',.47)
...
if not CLAY:export('morfeu-rigged-v05.glb',[rig]+catparts)
rig.scale=(.58,)*3;rig.location=(0,.10,.40)
if not CLAY:export('morfeu-scout-v05.glb',[rig]+catparts+[shiproot]+ship)
