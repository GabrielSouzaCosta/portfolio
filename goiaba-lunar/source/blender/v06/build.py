"""Morfeu v06: natural orange eyes with an integrated orbit and optical reflections.
All geometry is authored locally in Blender. The compact character and original
Semente scout are preserved; the owner's eye feedback supersedes square catches.
"""
...
COLORS={'cream':'FFF9F0','orange':'D67525','cap':'D97829','stripe':'9E491C','ink':'493426','pupil':'241C18','pink':'EBA392','gold':'EB8A25','ivory':'DDD8C6','hull':'ECE6D5','teal':'243D42','metal':'535E60','rust':'C25C30','blue':'81DEEF'}
def rgba(hex):
...
def headcolor(p):
 ...
 base=blend(gingerlight,darkginger,stripes*.46)
...
lids=[]
# The iris is circular in the final head coordinates, independently of the
# almond-shaped eye opening. The lids crop the disk, not deform its pattern.
# Reflections come from the optical material and lights, never painted squares.
IW=512;IH=512;pixels=[]
for j in range(IH):
 v=(j+.5)/IH
 for i in range(IW):
  u=(i+.5)/IW;dx=(u-.5)*2;dz=(v-.5)*2
  x=dx*EYE_W*1.14;z=dz*EYE_H*.93-.009
  r=math.sqrt(x*x+z*z)/.151;t=math.atan2(z,x)
  fibers=(math.sin(t*53+math.sin(t*11)*2+r*13)+.5*math.sin(t*89-r*19)+.4*math.sin(t*137+r*7))/1.9
  iris=blend(rgba('F0C27B'),rgba('CB944F'),.30+.20*smooth(.35,.96,r)+.018*fibers)
  iris=blend(iris,rgba('96754B'),smooth(.92,1.01,r)*.40)
  # White sclera with a neutral, subtle edge shadow in the lid corners.
  sclera=blend(rgba('FFFFFF'),rgba('EEEEEE'),smooth(.95,1.36,r)*.45)
  c=blend(iris,sclera,smooth(.985,1.018,r))
  q=(x/.067)**2+(z/.103)**2
  c=blend(rgba('131916'),c,smooth(.965,1.045,q))
  # Soft lid occlusion gives depth even under the preview's fill illumination.
  shade=.12*smooth(.28,.95,dz)+.10*smooth(.82,1.04,math.sqrt(dx*dx+dz*dz))
  c=tuple(channel*(1-shade) for channel in c[:3])+(1,)
  pixels.extend(c)
tex=bpy.data.images.new('Morfeu circular orange iris and vertical pupil',IW,IH);tex.pixels=pixels;tex.filepath_raw=str(P/'morfeu-eyes.png');tex.file_format='PNG';tex.save();tex.pack()
eye_material=mat('Morfeu | moist amber eyes','gold',.16)
bs=eye_material.node_tree.nodes.get('Principled BSDF');bs.inputs['Specular IOR Level'].default_value=.28;bs.inputs['Coat Weight'].default_value=.12;bs.inputs['Coat Roughness'].default_value=.10;bs.inputs['IOR'].default_value=1.38
...
for s,label in [(-1,'L'),(1,'R')]:
 ...
 ax=(yr-yl)/(2*EYE_W)*.45;az=(yt-yb)/(2*EYE_H)*.65
 ...
 for vertex in head.data.vertices:
  ...
  radius=math.sqrt((dx/EYE_W)**2+abs(dz/EYE_H)**(2/1.3))
  if radius<1.80:
   weight=(1-smooth(1.20,1.80,radius))*(1-smooth(-.15,.06,vertex.co.y))
 ...
 for j in range(R+1):
  ...
  for i in range(N+1):
   ...
   # A convex cornea in both axes produces compact reflections. Keep its
   # centre behind the closed lids and recess its perimeter into the socket.
   optical=Vector(point(dx,dz));optical.y+=.010+.038*r*r
 ...
 for upper in [True,False]:
  vs=[];closed=[];fs=[];NX=72
  rows=[0,.075,.15,.225,.30,.375,.45,.525,.60,.675,.75,.815,.88,.93,.965,.985,1];NY=len(rows)-1
  for j,v in enumerate(rows):
   for i in range(NX+1):
    ...
    innerz=edge*(.79 if upper else .94)+(-.004 if upper else .003)*math.sqrt(max(0,1-u*u))
    openz=outerz*(1-v)+innerz*v
    op=Vector(point(outerdx*(1-v)+dx*v,openz,.004))
    # The upper margin overlaps the lower by a narrow, continuous crease.
    clz=outerz*(1-v)+((-.025 if upper else -.021)*math.sqrt(max(0,1-u*u)))*v
    ...
    for co in [op,cp]:
     ...
     # Stay flush at the actual almond cut boundary in BOTH poses. Only the
     # apron outside the opening sinks beneath the skin; its join is tangent.
     boundary=math.sqrt((ddx/EYE_W)**2+abs(ddz/EYE_H)**(2/1.3))
     co.y=point(ddx,ddz)[1]+.0075+.012*smooth(1.04,1.18,boundary)
    if upper:cp.y-=.0012*smooth(.78,1,v)
  ...
  for i,co in enumerate(closed):
   color=headcolor(co)
   if upper and i//(NX+1)>=NY-1:
    u=(i%(NX+1))/NX*2-1
    amount=(.60 if i//(NX+1)==NY else .14)*(1-smooth(.88,1,abs(u)))
    color=blend(color,rgba('604C3C'),amount)
...
for s,label in [(-1,'L'),(1,'R')]:
 tube('Mouth cleft '+label,[(0,-.56,2.022),(s*.048,-.563,2.013),(s*.078,-.565,2.024),(s*.101,-.556,2.042)],.005,mouthmat)
 for k in range(2):
...
scene.frame_set(1)
# Original Semente craft is authored locally from its own hull and mechanisms.
exec(compile((P/'ship.py').read_text(),str(P/'ship.py'),'exec'))
...
if not CLAY:export('morfeu-rigged-v06.glb',[rig]+catparts)
rig.scale=(.58,)*3;rig.location=(0,.10,.40)
if not CLAY:export('morfeu-scout-v06.glb',[rig]+catparts+[shiproot]+ship)
...
for name,loc,power,size,color in [('Key',(-3,-4,6),470,4,(1,.96,.9)),('Fill',(4,-2,4),240,3,(.8,.9,1)),('Rim',(0,4,6),650,3,(1,.96,.88))]:
 bpy.ops.object.light_add(type='AREA',location=loc);o=bpy.context.object;o.name=name;o.data.energy=power;o.data.shape='DISK';o.data.size=size;o.data.color=color;o.data.specular_factor=.04;aim(o,(0,0,1.5))
# Small, real light sources produce optical catches without washing out the iris.
for name,loc,power,size in [('Eye key',(-2.5,-4.5,4.5),80,.45),('Eye fill',(3,-4.5,3),18,.35)]:
 bpy.ops.object.light_add(type='AREA',location=loc);o=bpy.context.object;o.name=name;o.data.energy=power;o.data.shape='DISK';o.data.size=size;o.data.color=(1,.98,.93);o.data.diffuse_factor=0;aim(o,(0,-.1,1.55))
...
for name,loc in [('eyes-front',(0,-8,1.7)),('eyes-quarter',(3.1,-7,2.05))]:
 cam.location=loc;aim(cam,(0,-.12,1.57));scene.render.filepath=str(P/'previews'/f'{name}.png');bpy.ops.render.render(write_still=True)
scene.frame_set(52);cam.location=(0,-8,1.7);aim(cam,(0,-.12,1.57));scene.render.filepath=str(P/'previews/eyes-blink.png');bpy.ops.render.render(write_still=True);scene.frame_set(1)
rig.scale=(.58,)*3;rig.location=(0,.10,.40)
for o in ship:o.hide_render=False;o.hide_set(False)
for name in ['Key','Fill','Rim']:bpy.data.objects[name].data.specular_factor=1
cam.data.ortho_scale=5.35
for name,loc,target in [('morfeu-scout',(4.8,-7,5.3),(0,-.2,.7)),('scout-rear',(-4.8,7,4.5),(0,-.05,.7)),('scout-top',(0,-.2,9),(0,-.2,0))]:
 cam.location=loc;aim(cam,target);scene.render.filepath=str(P/'previews'/f'{name}.png');bpy.ops.render.render(write_still=True)
